...
let dec_color_pair x = (x mod color_num, x / color_num)

let init_colors io:io = begin
        ignore (Curses.start_color ());
        for bg = 0 to 7 do
            for fg = 0 to 7 do
                ignore (Curses.init_pair (enc_color_pair (fg, bg)) fg bg)
            done
        done;
        io
    end
...
let quit io = Curses.endwin (); io
let refresh io = ignore (Curses.refresh ()); io
let size io =
...
let box io =
    let color = enc_color_pair (color_white, color_black) in begin
...
let rec print_char col c (x, y) io = match col with
    | None -> ignore (Curses.mvaddch y x (int_of_char c)); io
    | Some (fg, bg) -> let color = enc_color_pair (fg, bg) in begin
        Curses.attr_set 0 color;
        print_char None c (x, y) io
    end

let rec print_string col str (x, y) io = match col with
    | None -> ignore (Curses.mvaddstr y x str); io
    | Some (fg, bg) -> let color = enc_color_pair (fg, bg) in begin
        Curses.attr_set 0 color;
        print_string None str (x, y) io
    end
